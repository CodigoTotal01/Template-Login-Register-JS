...
function registrarCuenta(e) {
    ...
    const fenaci = document.querySelector("#fenaci").value;
    //Validar un raio buton, 
    const usuario = document.querySelector("#usuario").value;
    const email = document.querySelector("#e-mail").value;
    const password = document.querySelector("#password").value;
    const confirm_pasword = document.querySelector("#confirmar_password").value;
    const genero_seleccionado = document.querySelector('input[name="genero"]:checked').value;
    //para saber si todos los campos estas completos
    if (nombre == "" || apellidos == "" || dni == "" || fenaci == "" || usuario == "" || email == "" || password == "" || confirm_pasword == "" || genero_seleccionado == "") {
        mostrarMensaje("error", "Falta rellenar los campos restantes🤔", "Verifica que todos los campos esten completos o recarge la pagina");
        return;
    } else {
        //arreglo de errores 
        ...
        //! No necesita validarce el de genero porque ya tiene uno por default
        if (usuario.length < 4) {
        ...
        if(password.length>7){
            if (confirm_pasword.length > 7 && confirm_pasword == password) {
                console.log("La contraselña es correcta :D ")
            } else {
